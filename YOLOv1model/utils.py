...
def gen_data_csv(image_directory, label_directory, test_size=10):
    ...
    Args:
        ...
        test_size: (int) percent of data to be held out for testing (default = 10)
...
def get_bboxes(
    ...
    # make sure YOLOmodel is in eval before get bboxes
    model.eval()
    train_idx = 0
    ...
    for batch_idx, (x, labels) in enumerate(loader):
        ...
        true_bboxes = cellboxes_to_boxes(labels, S=model.S, B=model.B, C=model.C)
        bboxes = cellboxes_to_boxes(predictions, S=model.S, B=model.B, C=model.C)
